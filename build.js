...
const homeTpl = fs.readFileSync("src/templates/home.mustache", "utf8");
...
fs.writeFileSync(
  "out/index.html",
  Mustache.render(base, { pageTitle: "Justin Liang", isHome: true, channels }, { content: homeTpl })
);
...
for (const channel of channels) {
  ...
  const channelsForPage = channels.map((c) => ({ ...c, isActive: c.slug === channel.slug }));

  fs.writeFileSync(
    path.join(dir, "index.html"),
    Mustache.render(
      base,
      { pageTitle: `${channel.name} — Justin Liang`, channels: channelsForPage, embedUrl: channel.embedUrl },
      { content: channelTpl }
    )
  );
